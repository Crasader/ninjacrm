...
function comboboxHighlighter(item) {
  ...
  result = result.replace(new RegExp("(" + query + ")", "ig"), function(
    $1,
    match
  ) {
...
function calculateAmounts(invoice) {
  ...
  for (var i = 0; i < invoice.invoice_items.length; i++) {
    ...
    var lineTotal = roundSignificant(
  ...
  invoice.total_amount = roundToTwo(
    roundToTwo(total) -
      (roundToTwo(invoice.amount) - roundToTwo(invoice.balance))
  );
    ...
    if (fuzzy) {
      ...
      $.each(data, function(i, obj) {
        if (substrRegex.test(obj[key])) {
          matches.push(obj);
        } else if (secondKey && substrRegex.test(obj[secondKey])) matches.push(obj);
      });
    }
    cb(matches);
  };
